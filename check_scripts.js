...
window.applyPricingToUI = function(cfg) {
  ...
  const pReveal = Number((_b = (_a = c.prices) == null ? void 0 : _a.reveal) != null ? _b : 59);
  const pMatch = Number((_d = (_c = c.prices) == null ? void 0 : _c.match) != null ? _d : 99);
  const pQuestion = Number((_f = (_e = c.prices) == null ? void 0 : _e.question) != null ? _f : 29);
  const isOffer = Boolean(((_g = c.offer) == null ? void 0 : _g.enabled) && Number((_h = c.offer) == null ? void 0 : _h.percent) > 0);
  const baseReveal = Number(((_i = c.basePrices) == null ? void 0 : _i.reveal) || pReveal);
  const baseMatch = Number(((_j = c.basePrices) == null ? void 0 : _j.match) || pMatch);
  const baseQuestion = Number(((_k = c.basePrices) == null ? void 0 : _k.question) || pQuestion);
  if (window.PAYWALL_CONFIG && window.PAYWALL_CONFIG.plans) {
    if (window.PAYWALL_CONFIG.plans.reveal)
      window.PAYWALL_CONFIG.plans.reveal.amountINR = pReveal;
    if (window.PAYWALL_CONFIG.plans.match)
      window.PAYWALL_CONFIG.plans.match.amountINR = pMatch;
    if (window.PAYWALL_CONFIG.plans.question)
      window.PAYWALL_CONFIG.plans.question.amountINR = pQuestion;
  }
  ...
  if (genBtn && !window.isVipUser) {
    ...
    genBtn.textContent = `${baseText} \xB7 \u20B9${pReveal}`;
  }
  ...
  if (matchBtn && !window.isVipUser) {
    ...
    matchBtn.textContent = `${baseText} \xB7 \u20B9${pMatch}`;
  }
  const chatHintSpan = document.querySelector(".chat-status span");
  if (chatHintSpan) {
    const isHi = window.currentVedicLang === "hi";
    chatHintSpan.textContent = isHi ? `\u0915\u0941\u0902\u0921\u0932\u0940, \u0926\u0936\u093E, \u092F\u094B\u0917, \u0938\u0902\u092C\u0902\u0927, \u0915\u0930\u093F\u092F\u0930 \u092F\u093E \u091C\u0940\u0935\u0928 \u0915\u0947 \u091A\u0930\u0923\u094B\u0902 \u0915\u0947 \u092C\u093E\u0930\u0947 \u092E\u0947\u0902 \u092A\u094D\u0930\u0936\u094D\u0928 \u092A\u0942\u091B\u0947\u0902\u0964 \u092A\u094D\u0930\u0924\u094D\u092F\u0947\u0915 \u092A\u094D\u0930\u0936\u094D\u0928 \u0915\u093E \u092E\u0942\u0932\u094D\u092F \u20B9${pQuestion} \u0939\u0948\u0964` : `Ask specific questions about the chart, Dasha, Yogas, relationships, career, or life phases. Each question costs \u20B9${pQuestion}.`;
  }
  ...
  planCards.forEach((card) => {
    ...
    if (plan === "reveal") {
      price = pReveal;
      basePrice = baseReveal;
    } else if (plan === "match") {
      price = pMatch;
      basePrice = baseMatch;
    } else if (plan === "question") {
      price = pQuestion;
      basePrice = baseQuestion;
...
function buildAtAGlance2() {
  ...
  if (!card || !fullReportText.trim())
    return;
  const t = cleanAstroText(fullReportText);
  const grab = (heading, nexts) => {
    const re = new RegExp("##\\s*" + heading + "\\s*\\n\\n([\\s\\S]*?)(?=\\n\\n##\\s*|$)", "i");
    const m = t.match(re);
    return m ? m[1].replace(/AREA\\s*\\|[\\s\\S]*/i, "").trim() : "";
  };
  ...
  const short = (x) => {
    ...
    return p.replace(/^[-•]+\s*/, "").slice(0, 330);
  };
  const rows = [
...
const SECTIONS = [
  {
    id: "overview",
    title: "1. Chart Overview & Core Placements",
  ...
  {
    id: "identity_health",
    title: "2. Identity, Temperament & Vitality",
  ...
  {
    id: "relationships",
    title: "3. Love, Marriage, Family & Social Life",
  ...
  {
    id: "career_wealth",
    title: "4. Career, Wealth & Material Success",
  ...
  {
    id: "timing_dashas",
    title: "5. Life Phases, Vimshottari Dasha & Transits",
  ...
  {
    id: "synthesis",
    title: "6. Synthesis & Life Purpose",
...
const KUNDLI_SECTIONS = [
  {
    id: "ashtakoot",
    title: "Ashtakoot Guna Milan (36-Point Compatibility)",
  ...
  {
    id: "doshas",
    title: "Mangal Dosha and Compatibility Doshas",
  ...
  {
    id: "synthesis",
    title: "Compatibility Outlook and Synthesis",
...
function verifyIndividualChart() {
  return __async(this, null, function* () {
    ...
    if (!Number.isFinite(lat) || !Number.isFinite(lon))
      throw new Error("A confirmed latitude and longitude are required for the precise ephemeris calculation.");
...
function verifyKundliCharts() {
  return __async(this, null, function* () {
    const get = (prefix, label) => __async(this, null, function* () {
      ...
      if (!Number.isFinite(lat) || !Number.isFinite(lon))
        throw new Error(`${label}: confirmed latitude and longitude are required for the precise ephemeris calculation.`);
      return fetchEphemerisChart({ date, time, lat, lon, name: document.getElementById(prefix + "_name").value || label });
    });
    verifiedCharts.partnerA = yield get("k1", "Male partner");
    verifiedCharts.partnerB = yield get("k2", "Female partner");
    birthContext += `
...
function rawCall(model, key, systemText, userText, maxTokens) {
  return __async(this, null, function* () {
    let res, data;
    try {
...
function callGeminiStream(systemText, userText, maxTokens, onChunk) {
  return __async(this, null, function* () {
    const maxAttempts = 3;
    let lastErr;
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        const res = yield fetch("/api/ai-stream", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        lastErr = err;
        if (err == null ? void 0 : err.isAuth)
          throw err;
        if (((err == null ? void 0 : err.modelNotFound) || (err == null ? void 0 : err.status) === 404) && activeModel !== FALLBACK_MODEL) {
          activeModel = FALLBACK_MODEL;
          continue;
        }
        const transient = (err == null ? void 0 : err.status) === 429 || (err == null ? void 0 : err.status) === 504 || (err == null ? void 0 : err.status) === 503 || (err == null ? void 0 : err.status) === 502 || (err == null ? void 0 : err.status) === 500 || (err == null ? void 0 : err.status) === 408 || (err == null ? void 0 : err.status) === 0;
        if (transient && attempt < maxAttempts) {
          yield sleep(attempt * 1200);
          continue;
        }
        throw err;
      }
...
function callGemini(systemText, userText, maxTokens = 3200) {
  return __async(this, null, function* () {
    const maxAttempts = 3;
    let lastErr;
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        return yield rawCall(activeModel, activeKey, systemText, userText, maxTokens);
      } catch (err) {
        lastErr = err;
        if (err == null ? void 0 : err.isAuth)
          throw err;
...
function renderPanchangReportCard() {
  ...
  if (!card)
    return;
  const dob = (_a2 = document.getElementById("f_dob")) == null ? void 0 : _a2.value;
  const lat = parseFloat((_b2 = document.getElementById("f_lat")) == null ? void 0 : _b2.value) || 28.6139;
  const lon = parseFloat((_c2 = document.getElementById("f_lon")) == null ? void 0 : _c2.value) || 77.209;
  const pData = getDailyPanchangData(dob ? new Date(dob) : new Date(), lat, lon);
  if (!pData) {
    card.style.display = "none";
    return;
  ...
  card.innerHTML = `
    <div class="panchang-report-box">
      <div class="panchang-report-header">
        ...
        <small>${pData.dateStr} \xB7 ${pData.hinduCal.hinduDateFormatted}</small>
      </div>
      <div class="panchang-report-grid">
        <div class="shubh-box"><small>\u{1F4DC} HINDU CALENDAR</small><b>${pData.hinduCal.maas} \xB7 ${pData.hinduCal.vikramSamvat}</b></div>
...
function renderDashaTimeline() {
  ...
  if (h3)
    h3.textContent = isHi ? "\u0938\u0915\u094D\u0930\u093F\u092F \u0935\u093F\u0902\u0936\u094B\u0924\u094D\u0924\u0930\u0940 \u0926\u0936\u093E \u0935 \u091C\u0940\u0935\u0928 \u0915\u093E\u0932\u0916\u0902\u0921" : "Interactive Dasha & Life-Phase Timeline";
...
function generateSection(index, reportBody, stepListEl, progressErrorEl) {
  return __async(this, null, function* () {
    ...
    `;
      ...
      if (tagEl2) {
        tagEl2.className = "report-chapter-tag ai-generating";
        tagEl2.textContent = isHi ? "\u2728 AI \u0935\u093F\u0936\u094D\u0932\u0947\u0937\u093F\u0924 \u0915\u093F\u092F\u093E \u091C\u093E \u0930\u0939\u093E \u0939\u0948\u2026" : "\u2728 Generating chapter via AI\u2026";
      }
...
- Do not repeat the chapter title at the very start.${resumePrompt}`;
        ...
        const streamHandler = (chunkText, fullSoFar) => {
          ...
          const now = Date.now();
          if (now - currentChunkRenderTime > 200 && finalContentDiv2) {
            currentChunkRenderTime = now;
            finalContentDiv2.innerHTML = formatReportSectionHtml(combined) + ' <span class="loading-gem-spinner" style="display:inline-block;font-size:12px;">\u2726</span>';
          }
        };
        const rawText = yield callGeminiStream(activeRules, userText, 3200, streamHandler);
  ...
  return __async(this, null, function* () {
    ...
    if (startIndex === 0) {
      try {
        ...
        e.className = "error";
        e.textContent = "Ephemeris calculation stopped: " + err.message;
    ...
    for (let i = startIndex; i < (endIndex != null ? endIndex : activeSections.length); i++) {
      yield generateSection(i, reportBody, stepListEl, progressErrorEl);
      if (i + 1 < (endIndex != null ? endIndex : activeSections.length)) {
        yield sleep(300);
      }
    }
...
document.getElementById("chatSend").onclick = () => __async(this, null, function* () {
  ...
  if (!q)
    return;
  if (!chatUnlocked) {
    alert("Wait for the chart to begin revealing before asking a question.");
    return;
  }
  if (chatQuestionsUsed >= MAX_CHAT_QUESTIONS) {
    alert("You have used all 5 questions for this reading. End the reading to start a new paid reading.");
    return;
  }
  if (!(yield window.requestPaidAccess("question")))
    return;
  chatQuestionsUsed++;
  updateChatCount();
  input.value = "";
  ...
  let astroDetails = "";
  if (verifiedChart) {
    astroDetails = `Ascendant (Lagna): ${verifiedChart.ascSign} at ${(_a2 = verifiedChart.ascDeg) == null ? void 0 : _a2.toFixed(1)}\xB0
Moon: ${verifiedChart.moonRashi} (${verifiedChart.nakshatra} Nakshatra, Pada ${verifiedChart.pada})
Sun: ${verifiedChart.sunRashi}
Active Vimshottari Cycle: ${((_b2 = verifiedChart.dasha) == null ? void 0 : _b2.activeMahadasha) || "Jupiter"} Mahadasha / ${((_c2 = verifiedChart.dasha) == null ? void 0 : _c2.activeAntardasha) || "Saturn"} Antardasha
...
(function() {
  ...
  function adminFetch(_0) {
    return __async(this, arguments, function* (url, opts = {}) {
      ...
      opts.headers = __spreadProps(__spreadValues({}, opts.headers || {}), { Authorization: "Bearer " + adminToken, "Content-Type": "application/json" });
  ...
  function renderOverview(reports, feedback, vips, s, payments, geminiQuota = {}) {
    ...
    const activeModelName = (geminiQuota == null ? void 0 : geminiQuota.primaryModel) || "gemini-3.6-flash";
...
(function() {
  ...
  let questionCredit = false;
  let vipAccess = false;
  function enabled(plan) {
    var _a;
    const f = ((_a = window.SERVER_CONFIG) == null ? void 0 : _a.features) || {};
    return plan === "reveal" ? f.reveal !== false : plan === "match" ? f.match !== false : f.chat !== false;
  }
  function pay(plan, customAmount, customPrefill) {
    return __async(this, null, function* () {
      if (vipAccess && plan !== "dakshina")
        return true;
      ...
      if (plan === "match" && entitlements.match)
        return true;
      if (plan === "question" && questionCredit)
        return true;
      ...
      try {
        ...
        return yield new Promise((resolve) => {
          ...
          const prefillName = (customPrefill == null ? void 0 : customPrefill.name) || ((_a = document.getElementById("f_name")) == null ? void 0 : _a.value) || ((_b = document.getElementById("k1_name")) == null ? void 0 : _b.value) || "";
          const prefillEmail = (customPrefill == null ? void 0 : customPrefill.email) || ((_c = document.getElementById("f_email")) == null ? void 0 : _c.value) || "";
          const options = {
            key,
            amount,
  ...
  window.requestPaidAccess = pay;
  window.consumeQuestionCredit = () => {
    if (!questionCredit)
      return false;
    questionCredit = false;
    return true;
  };
  window.resetPaymentSession = () => {
...
const OmChantEngine = function() {
  ...
  function startSynthDrone(presetKey) {
    ...
    masterGainNode.gain.setValueAtTime(1e-4, now);
    ...
    if (presetKey === "solar_528") {
      freqs = [132, 264, 528, 792];
    } else if (presetKey === "singing_bowl") {
      freqs = [144, 288, 576, 864, 1152];
    } else if (presetKey === "temple_bells") {
      freqs = [319.5, 639, 1278, 1917, 2556];
    }
  ...
  function strikeTestSound() {
    ...
    harmonics.forEach((f, i) => {
      ...
      gain.gain.setValueAtTime(Math.max(1e-4, startLevel), now);
      gain.gain.exponentialRampToValueAtTime(1e-4, now + 3);
      osc.connect(gain);
  ...
  function setVolume(vol) {
    ...
    if (lbl)
      lbl.textContent = `${Math.round(masterVol * 100)}%`;
  }
...
function openEventDetails(eventName) {
  ...
  if (mIcon)
    mIcon.textContent = info.icon;
  if (mTitle)
    mTitle.textContent = info.title;
  if (mCat)
    mCat.textContent = info.category + " \xB7 " + info.subtitle;
  if (mSig)
    mSig.textContent = info.significance;
  if (mRit)
    mRit.textContent = info.rituals;
  if (mBle)
    mBle.textContent = info.blessing;
  modal.classList.add("open");
}
...
function openPanchangModal() {
  ...
  if (!modal)
    return;
  const lat = parseFloat((_a = document.getElementById("f_lat")) == null ? void 0 : _a.value) || 28.6139;
  ...
  if (titleEl)
    titleEl.textContent = `Today's Dainik Panchang \u2014 ${pData.dayOfWeek}`;
  if (dateEl)
    dateEl.textContent = `${pData.dateStr} \xB7 ${pData.hinduCal.hinduDateFormatted} \xB7 ${pData.hinduCal.maas} \xB7 ${pData.hinduCal.vikramSamvat} \xB7 Observer (${lat.toFixed(2)}\xB0N, ${lon.toFixed(2)}\xB0E)`;
  if (contentEl) {
    contentEl.innerHTML = `
      <div class="pdt-grid">
        <div class="pdt-card">
          <span class="pdt-card-badge">\u{1F4DC} 1. TITHI (LUNAR DAY)</span>
        ...
        <div class="pdt-card">
          <span class="pdt-card-badge">\u{1F300} 3. YOGA (SOLAR-LUNAR SUM)</span>
        ...
        <div class="pdt-card">
          <span class="pdt-card-badge">\u{1F4C5} 5. VARA (SOLAR DAY)</span>
        ...
        <div class="pdt-card">
          <span class="pdt-card-badge">\u{1F305} SUN & CELESTIAL TIMINGS</span>
          <span class="pdt-card-title" style="font-size: 15px;">Sunrise: ${pData.sun.sunrise} \xB7 Sunset: ${pData.sun.sunset}</span>
          <div class="pdt-card-desc">
      ...
      <div style="margin-top: 20px; padding: 20px; background: rgba(223, 186, 109, 0.06); border: 1px solid rgba(223, 186, 109, 0.3); border-radius: 14px;">
        <h3 style="font-family: 'Cinzel', serif; font-size: 17px; color: #f7e4b8; margin: 0 0 12px 0;">\u{1F4CA} TODAY'S CHOGHADIYA TIMINGS</h3>
      ...
      ${pData.activeEvents.length > 0 || pData.upcomingEvents.length > 0 ? `
        <div style="margin-top: 20px;">
          <h3 style="font-family: 'Cinzel', serif; font-size: 17px; color: #7fc5c0; margin: 0 0 10px 0;">\u{1F33A} TODAY & UPCOMING FESTIVALS</h3>
...
function openSpecificPanchangDetail(type) {
  ...
  const detailMap = {
    tithi: {
      ...
      content: `
        <div style="background: rgba(216, 160, 76, 0.1); border: 1px solid rgba(216, 160, 76, 0.3); border-radius: 12px; padding: 16px; margin-bottom: 16px;">
          ...
          <div style="font-size: 13px; color: #a3b6be; margin-top: 4px;">Paksha: <b>${pData.paksha} Paksha</b> \xB7 Vikram Samvat 2081</div>
        </div>
        <p style="color: #d1dfd8; line-height: 1.6; font-size: 14px;">
          <b>Spiritual Significance:</b> Tithi represents the distance between the Sun and Moon (each 12\xB0 phase). It governs emotional stability, relationship dynamics, and karmic timing for daily activities.
        </p>
    ...
    brahma: {
      ...
      content: `
        <div style="background: rgba(241, 212, 138, 0.15); border: 1px solid rgba(241, 212, 138, 0.4); border-radius: 12px; padding: 16px; margin-bottom: 16px;">
          <div style="font-size: 20px; font-weight: bold; color: #f1d48a; font-family: 'Cinzel', serif;">\u{1F9D8} 04:24 AM \u2013 05:12 AM</div>
    ...
    sun: {
      ...
      title: `Sunrise: ${pData.sun.sunrise} \xB7 Sunset: ${pData.sun.sunset}`,
      content: `
        <div style="background: rgba(216, 160, 76, 0.15); border: 1px solid rgba(216, 160, 76, 0.4); border-radius: 12px; padding: 16px; margin-bottom: 16px;">
          <div style="font-size: 18px; font-weight: bold; color: #f1d48a; font-family: 'Cinzel', serif;">\u{1F305} Sunrise: ${pData.sun.sunrise}</div>
          <div style="font-size: 18px; font-weight: bold; color: #f1d48a; font-family: 'Cinzel', serif; margin-top: 6px;">\u{1F307} Sunset: ${pData.sun.sunset}</div>
...
window.addEventListener("load", () => {
  ...
  loadCurrentSky();
  setInterval(loadCurrentSky, 12e4);
});
...
window.selectPaymentPlan = function(plan) {
  ...
  if (btn)
    btn.textContent = `Proceed to Secure Payment (\u20B9${amt})`;
};
document.addEventListener("DOMContentLoaded", () => {
  var _a, _b;
  document.querySelectorAll("#paymentPlansGrid .plan-card").forEach((card) => {
  ...
  document.querySelectorAll("#dakshinaPresets .dakshina-chip").forEach((chip) => {
    chip.addEventListener("click", () => {
      ...
      const btn = document.getElementById("payProceedBtn");
      if (btn)
        btn.textContent = `Proceed to Secure Payment (\u20B9${val})`;
    });
  });
  (_a = document.getElementById("payProceedBtn")) == null ? void 0 : _a.addEventListener("click", () => __async(this, null, function* () {
    ...
    try {
      ...
      const success = yield window.requestPaidAccess(plan);
      if (success) {
        if (status) {
  ...
  (_b = document.getElementById("premiumUnlockBtn")) == null ? void 0 : _b.addEventListener("click", () => {
...
window.setVedicTheme = function(themeName) {
  ...
  if (labelEl)
    labelEl.textContent = themeLabels[themeName] || "Cosmic Night";
...
window.setVedicLanguage = function(lang) {
  ...
  if (btnText)
    btnText.textContent = isHi ? "English" : "\u0939\u093F\u0902\u0926\u0940";
  if (titleText)
    titleText.textContent = isHi ? "\u091C\u094D\u092F\u094B\u0924\u093F\u0937 \u0935\u093F\u092E\u0930\u094D\u0936" : "JYOTISH VIMARSHA";
  if (subtitleText)
    subtitleText.textContent = isHi ? "\u092A\u0935\u093F\u0924\u094D\u0930 \u0935\u0948\u0926\u093F\u0915 \u0915\u0941\u0902\u0921\u0932\u0940 \u090F\u0935\u0902 \u092A\u094D\u0930\u0924\u094D\u092F\u0915\u094D\u0937 \u0917\u094D\u0930\u0939 \u0917\u0923\u0928\u093E" : "Sacred Vedic Chart Reading & Ephemeris";
  if (feedbackBtnText)
    feedbackBtnText.textContent = isHi ? "\u0938\u0941\u091D\u093E\u0935 \u0935 \u0938\u0902\u092A\u0930\u094D\u0915" : "Feedback";
  if (glossaryBtnText)
    glossaryBtnText.textContent = isHi ? "\u092A\u093E\u0930\u093F\u092D\u093E\u0937\u093F\u0915 \u0936\u092C\u094D\u0926\u093E\u0935\u0932\u0940" : "Glossary";
  const hudSpans = document.querySelectorAll(".observatory-telemetry-hud > span");
  if (hudSpans && hudSpans.length >= 2) {
    hudSpans[0].textContent = isHi ? `\u0916\u0917\u094B\u0932\u0940\u092F \u0935\u0947\u0927\u0936\u093E\u0932\u093E \xB7 \u092A\u094D\u0930\u0924\u094D\u092F\u0915\u094D\u0937 \u0932\u093E\u0939\u093F\u0930\u0940 \u0905\u092F\u0928\u093E\u0902\u0936 24\xB010'38"` : `CELESTIAL OBSERVATORY \xB7 SIDEREAL LAHIRI AYANAMSHA 24\xB010'38"`;
    hudSpans[1].textContent = isHi ? "\u0935\u0947\u0927\u0936\u093E\u0932\u093E \u0905\u0915\u094D\u0937\u093E\u0902\u0936 28\xB037'\u0909 \u2022 \u0930\u0947\u0916\u093E\u0902\u0936 77\xB013'\u092A\u0942 \u2609" : "OBSERVER LAT 28\xB037'N \u2022 LON 77\xB013'E \u2609";
  }
  ...
  if (panchangBadge)
    panchangBadge.textContent = isHi ? "\u2726 \u0906\u091C \u0915\u093E \u0926\u0948\u0928\u093F\u0915 \u092A\u0902\u091A\u093E\u0902\u0917 \u0935 \u0930\u093E\u0939\u0941 \u0915\u093E\u0932" : "\u2726 TODAY'S DAINIK PANCHANG & RAHU KAAL";
  const panchangTrigger = document.querySelector(".panchang-detail-trigger-btn");
  if (panchangTrigger)
    panchangTrigger.textContent = isHi ? "\u{1F50D} \u0938\u0902\u092A\u0942\u0930\u094D\u0923 \u092A\u0902\u091A\u093E\u0902\u0917, \u091A\u094C\u0918\u0921\u093C\u093F\u092F\u093E \u0935 \u0936\u0941\u092D \u092E\u0941\u0939\u0942\u0930\u094D\u0924 \u0926\u0947\u0916\u0947\u0902 \u2726" : "\u{1F50D} View Complete Panchang & All Muhurtas \u2726";
  const dateCheckerLabel = document.querySelector(".panchang-date-picker-wrap span");
  if (dateCheckerLabel)
    dateCheckerLabel.textContent = isHi ? "\u{1F4C5} \u0924\u093F\u0925\u093F \u091A\u0941\u0928\u0947\u0902:" : "\u{1F4C5} Date Checker:";
  const liveTodayBtn = document.querySelector(".panchang-date-picker-wrap button");
  if (liveTodayBtn)
    liveTodayBtn.textContent = isHi ? "\u0906\u091C \u0915\u093E \u0926\u093F\u0928" : "Live Today";
  const panchangSmallLabels = {
    hinduCal: isHi ? "\u{1F4DC} \u0938\u0902\u0935\u0924 \u0935 \u092E\u093E\u0938 \u{1F50D}" : "\u{1F4DC} HINDU YEAR & MAAS \u{1F50D}",
    tithi: isHi ? "\u{1F319} \u0924\u093F\u0925\u093F \u0935 \u092A\u0915\u094D\u0937 \u{1F50D}" : "\u{1F319} TITHI & PAKSHA \u{1F50D}",
    nakshatra: isHi ? "\u2728 \u0928\u0915\u094D\u0937\u0924\u094D\u0930 \u0935 \u092A\u0926 \u{1F50D}" : "\u2728 NAKSHATRA & PADA \u{1F50D}",
    yoga: isHi ? "\u2638 \u092F\u094B\u0917 \u{1F50D}" : "\u2638 YOGA \u{1F50D}",
    karana: isHi ? "\u26A1 \u0915\u0930\u0923 \u{1F50D}" : "\u26A1 KARANA \u{1F50D}",
    rahuKaal: isHi ? "\u26A0\uFE0F \u0930\u093E\u0939\u0941 \u0915\u093E\u0932 (\u0905\u0936\u0941\u092D \u0915\u093E\u0932) \u{1F50D}" : "\u26A0\uFE0F RAHU KAAL (INAUSPICIOUS) \u{1F50D}",
    abhijit: isHi ? "\u{1F31F} \u0905\u092D\u093F\u091C\u093F\u0924 \u092E\u0941\u0939\u0942\u0930\u094D\u0924 (\u0905\u0924\u093F \u0936\u0941\u092D) \u{1F50D}" : "\u{1F31F} ABHIJIT MUHURTA (AUSPICIOUS) \u{1F50D}",
    sun: isHi ? "\u{1F305} \u0938\u0942\u0930\u094D\u092F\u094B\u0926\u092F \u0935 \u0938\u0942\u0930\u094D\u092F\u093E\u0938\u094D\u0924 \u{1F50D}" : "\u{1F305} SUNRISE & SUNSET \u{1F50D}",
    choghadiya: isHi ? "\u23F1 \u0926\u093F\u0928 \u0915\u093E \u091A\u094C\u0918\u0921\u093C\u093F\u092F\u093E \u{1F50D}" : "\u23F1 DAY CHOGHADIYA \u{1F50D}",
    events: isHi ? "\u{1FA94} \u092A\u0930\u094D\u0935, \u0935\u094D\u0930\u0924 \u0935 \u0924\u094D\u092F\u094C\u0939\u093E\u0930 \u{1F50D}" : "\u{1FA94} ACTIVE FESTIVAL & VRAT \u{1F50D}"
  };
  ...
  if (refBadge)
    refBadge.textContent = isHi ? "\u2726 \u0926\u0948\u0928\u093F\u0915 \u0916\u0917\u094B\u0932\u0940\u092F \u091A\u093F\u0902\u0924\u0928 \xB7 \u0935\u0948\u0926\u093F\u0915 \u091C\u094D\u091E\u093E\u0928\u093E\u092E\u0943\u0924" : "\u2726 DAILY CELESTIAL CONTEMPLATION \xB7 VEDIC WISDOM";
  ...
  if (introEyebrow)
    introEyebrow.textContent = isHi ? "\u2726 \u0915\u093E\u0932 \u091A\u0915\u094D\u0930 \u0915\u093E \u092E\u093E\u0928\u091A\u093F\u0924\u094D\u0930 \u2022 \u0915\u0930\u094D\u092E \u0915\u093E \u0926\u0930\u094D\u092A\u0923 \u2022 \u0906\u0915\u093E\u0936 \u0915\u0940 \u0926\u093F\u0935\u094D\u092F \u092D\u093E\u0937\u093E" : "\u2726 A MAP OF TIME \u2022 A MIRROR OF PATTERNS \u2022 A LANGUAGE OF THE SKY";
  const mysteryQuote = document.querySelector(".mystery-quote");
  if (mysteryQuote) {
    mysteryQuote.innerHTML = isHi ? "\u201C\u0915\u0941\u091B \u092A\u094D\u0930\u0936\u094D\u0928\u094B\u0902 \u0915\u0947 \u0909\u0924\u094D\u0924\u0930 \u0915\u0947\u0935\u0932 \u0906\u0917\u0947 \u0926\u0947\u0916\u0928\u0947 \u0938\u0947 \u0928\u0939\u0940\u0902 \u2014<br><em>\u0935\u0947 \u0906\u0915\u093E\u0936 \u0915\u0947 \u0938\u0902\u0915\u0947\u0924\u094B\u0902 \u0915\u094B \u0938\u092E\u091D\u0928\u0947 \u0938\u0947 \u092A\u094D\u0930\u093E\u092A\u094D\u0924 \u0939\u094B\u0924\u0947 \u0939\u0948\u0902\u0964</em>\u201D" : "\u201CSome questions are not answered by looking farther ahead \u2014<br><em>they begin by learning how to read the sky.</em>\u201D";
  }
  const introKicker = document.querySelector(".intro-kicker");
  if (introKicker)
    introKicker.textContent = isHi ? "\u092A\u094D\u0930\u093E\u091A\u0940\u0928 \u0906\u0915\u093E\u0936, \u0935\u0930\u094D\u0924\u092E\u093E\u0928 \u091C\u0940\u0935\u0928 \u0915\u0947 \u092A\u0930\u093F\u092A\u094D\u0930\u0947\u0915\u094D\u0937\u094D\u092F \u092E\u0947\u0902" : "THE ANCIENT SKY, READ FOR THE PRESENT MOMENT";
  ...
  if (introNote)
    introNote.textContent = isHi ? "\u0906\u092A\u0915\u0940 \u091C\u0928\u094D\u092E \u0924\u093F\u0925\u093F \u2022 \u0938\u092E\u092F \u2022 \u0938\u094D\u0925\u093E\u0928 \u0939\u0940 \u0907\u0938\u0915\u0940 \u0915\u0941\u0902\u091C\u0940 \u0939\u0948\u0902" : "Your date \u2022 time \u2022 place become the key";
  ...
  if (howTitle)
    howTitle.textContent = isHi ? "\u0938\u093E\u092E\u093E\u0928\u094D\u092F \u0926\u0948\u0928\u093F\u0915 \u0930\u093E\u0936\u093F\u092B\u0932 \u0938\u0947 \u092A\u0930\u0947" : "BEYOND A DAILY HOROSCOPE";
  ...
  if (dobLabel)
    dobLabel.childNodes[0].nodeValue = isHi ? "\u091C\u0928\u094D\u092E \u0924\u093F\u0925\u093F " : "Date of birth ";
  const tobLabel = document.querySelector("#setupCard label:has(#tob)");
  if (tobLabel)
    tobLabel.childNodes[0].nodeValue = isHi ? "\u091C\u0928\u094D\u092E \u0938\u092E\u092F " : "Time of birth ";
  const pobLabel = document.querySelector("#setupCard label:has(#pob)");
  if (pobLabel)
    pobLabel.childNodes[0].nodeValue = isHi ? "\u091C\u0928\u094D\u092E \u0938\u094D\u0925\u093E\u0928 (\u0936\u0939\u0930, \u0930\u093E\u091C\u094D\u092F, \u0926\u0947\u0936) " : "Place of birth ";
  const pobInput = document.getElementById("pob");
  if (pobInput)
    pobInput.placeholder = isHi ? "\u0909\u0926\u093E. \u0928\u0908 \u0926\u093F\u0932\u094D\u0932\u0940, \u092E\u0941\u0902\u092C\u0908, \u091C\u092F\u092A\u0941\u0930, \u0935\u093E\u0930\u093E\u0923\u0938\u0940, \u0932\u0902\u0926\u0928\u2026" : "e.g. New Delhi, Mumbai, Jaipur, London\u2026";
  const advCoordToggle = document.getElementById("advCoordToggle");
  if (advCoordToggle)
    advCoordToggle.textContent = isHi ? "\u2699 \u0905\u0915\u094D\u0937\u093E\u0902\u0936 / \u0926\u0947\u0936\u093E\u0902\u0924\u0930 (\u0935\u0948\u0915\u0932\u094D\u092A\u093F\u0915)" : "\u2699 Advanced Coordinates (optional)";
  const latLabel = document.querySelector("#setupCard label:has(#lat)");
  if (latLabel)
    latLabel.childNodes[0].nodeValue = isHi ? "\u0905\u0915\u094D\u0937\u093E\u0902\u0936 (\xB0N) " : "Latitude (\xB0N) ";
  const lonLabel = document.querySelector("#setupCard label:has(#lon)");
  if (lonLabel)
    lonLabel.childNodes[0].nodeValue = isHi ? "\u0926\u0947\u0936\u093E\u0902\u0924\u0930 (\xB0E) " : "Longitude (\xB0E) ";
  const tzLabel = document.querySelector("#setupCard label:has(#tz)");
  if (tzLabel)
    tzLabel.childNodes[0].nodeValue = isHi ? "\u0938\u092E\u092F \u0915\u094D\u0937\u0947\u0924\u094D\u0930 (\u0918\u0902\u091F\u0947) " : "Timezone Offset (hours) ";
  const consentLabel = document.querySelector("#setupCard .consent-check-row span");
  if (consentLabel) {
    consentLabel.innerHTML = isHi ? "\u092E\u0948\u0902 \u0938\u092E\u091D\u0924\u093E/\u0938\u092E\u091D\u0924\u0940 \u0939\u0942\u0901 \u0915\u093F \u092F\u0939 \u091C\u0928\u094D\u092E \u0915\u0941\u0902\u0921\u0932\u0940 \u0905\u0927\u094D\u092F\u092F\u0928 \u0935\u0948\u0926\u093F\u0915 \u091A\u093F\u0902\u0924\u0928 \u0914\u0930 \u0906\u0924\u094D\u092E-\u092E\u093E\u0930\u094D\u0917\u0926\u0930\u094D\u0936\u0928 \u0915\u0947 \u0932\u093F\u090F \u0939\u0948 \u2014 \u0915\u093F\u0938\u0940 \u091A\u093F\u0915\u093F\u0924\u094D\u0938\u093E, \u0915\u093E\u0928\u0942\u0928\u0940 \u092F\u093E \u0935\u093F\u0924\u094D\u0924\u0940\u092F \u0938\u0932\u093E\u0939 \u0915\u093E \u0935\u093F\u0915\u0932\u094D\u092A \u0928\u0939\u0940\u0902 \u0939\u0948\u0964" : "I understand this reading is for self-reflection and personal inquiry \u2014 not medical, legal, or financial advice.";
  }
  const genBtn = document.getElementById("genBtn");
  if (genBtn)
    genBtn.textContent = isHi ? "\u092A\u0935\u093F\u0924\u094D\u0930 \u091C\u0928\u094D\u092E \u0915\u0941\u0902\u0921\u0932\u0940 \u0917\u0923\u0928\u093E \u0915\u0930\u0947\u0902 \xB7 \u20B959" : "Reveal the chart \xB7 \u20B959";
  const matchBtn = document.getElementById("matchBtn");
  if (matchBtn)
    matchBtn.textContent = isHi ? "\u0917\u0941\u0923 \u092E\u093F\u0932\u093E\u0928 \u092A\u0930\u093F\u0923\u093E\u092E \u0926\u0947\u0916\u0947\u0902 \xB7 \u0928\u093F\u0903\u0936\u0941\u0932\u094D\u0915" : "View Guna Milan \xB7 Free";
  const unlockMatchBtn = document.getElementById("unlockMatchBtn");
  if (unlockMatchBtn)
    unlockMatchBtn.textContent = isHi ? "\u0935\u093F\u0938\u094D\u0924\u0943\u0924 \u0917\u0941\u0923 \u092E\u093F\u0932\u093E\u0928 \u0935 \u092E\u093E\u0902\u0917\u0932\u093F\u0915 \u0935\u093F\u0936\u094D\u0932\u0947\u0937\u0923 \u0905\u0928\u0932\u0949\u0915 \u0915\u0930\u0947\u0902 \xB7 \u20B999" : "Unlock detailed match \xB7 \u20B999";
  const astroYantraDiv = document.querySelector(".sacred-yantra-divider");
  if (astroYantraDiv)
    astroYantraDiv.textContent = isHi ? "\u2726 \u092A\u0935\u093F\u0924\u094D\u0930 \u0935\u0948\u0926\u093F\u0915 \u091C\u094D\u092F\u094B\u0924\u093F\u0937 \u0930\u0939\u0938\u094D\u092F \u090F\u0935\u0902 \u0916\u0917\u094B\u0932\u0940\u092F \u091C\u094D\u091E\u093E\u0928 \u2726" : "\u2726 SACRED VEDIC ASTRO FACTS & CELESTIAL WISDOM \u2726";
  const astroKicker = document.querySelector(".astro-kicker");
  if (astroKicker)
    astroKicker.textContent = isHi ? "\u0928\u093F\u0930\u092F\u0923 \u0916\u0917\u094B\u0932 \u0935\u093F\u091C\u094D\u091E\u093E\u0928 \u090F\u0935\u0902 \u092A\u093E\u0930\u093E\u0936\u0930\u0940 \u0938\u093F\u0926\u094D\u0927\u093E\u0902\u0924" : "SIDEREAL MECHANICS & CLASSICAL PARASHARI CONCEPTS";
  const astroH2 = document.querySelector(".astro-facts-header h2");
  if (astroH2)
    astroH2.textContent = isHi ? "\u0935\u0948\u0926\u093F\u0915 \u091C\u094D\u092F\u094B\u0924\u093F\u0937 \u0915\u0940 \u0906\u0927\u093E\u0930\u0936\u093F\u0932\u093E" : "Celestial Architecture at a Glance";
  const astroP = document.querySelector(".astro-facts-header p");
  if (astroP)
    astroP.textContent = isHi ? "\u0935\u0947 \u092E\u094C\u0932\u093F\u0915 \u0916\u0917\u094B\u0932\u0940\u092F \u0914\u0930 \u0935\u0948\u0926\u093F\u0915 \u0938\u093F\u0926\u094D\u0927\u093E\u0902\u0924 \u091C\u094B \u0906\u092A\u0915\u0940 \u091C\u0928\u094D\u092E \u0915\u0941\u0902\u0921\u0932\u0940 \u0914\u0930 \u092B\u0932\u0915\u0925\u0928 \u0915\u093E \u0906\u0927\u093E\u0930 \u092C\u0928\u0924\u0947 \u0939\u0948\u0902\u0964" : "Essential astronomical and Vedic astrological principles that form the foundation of your Kundli and planetary readings.";
  ...
  if (factCards.length >= 6) {
    ...
    factCards[0].querySelector("p").innerHTML = isHi ? `\u0935\u0948\u0926\u093F\u0915 \u091C\u094D\u092F\u094B\u0924\u093F\u0937 <strong>\u0928\u093F\u0930\u092F\u0923 \u0930\u093E\u0936\u093F \u091A\u0915\u094D\u0930</strong> \u0915\u093E \u0909\u092A\u092F\u094B\u0917 \u0915\u0930\u0924\u093E \u0939\u0948, \u091C\u094B \u0935\u093E\u0938\u094D\u0924\u0935\u093F\u0915 \u0928\u0915\u094D\u0937\u0924\u094D\u0930\u094B\u0902 \u0938\u0947 \u091C\u0941\u0921\u093C\u093E \u0939\u0948\u0964 \u092A\u0943\u0925\u094D\u0935\u0940 \u0915\u0940 \u0927\u0941\u0930\u0940 \u0939\u0930 72 \u0935\u0930\u094D\u0937\u094B\u0902 \u092E\u0947\u0902 ~1\xB0 \u091D\u0941\u0915\u0924\u0940 \u0939\u0948\u0964 \u0938\u093E\u092F\u0928 \u0914\u0930 \u0928\u093F\u0930\u092F\u0923 \u0930\u093E\u0936\u093F \u091A\u0915\u094D\u0930 \u0915\u0947 \u0905\u0902\u0924\u0930 \u0915\u094B <em>\u0905\u092F\u0928\u093E\u0902\u0936</em> \u0915\u0939\u0924\u0947 \u0939\u0948\u0902 (\u0935\u0930\u094D\u0924\u092E\u093E\u0928 \u092E\u0947\u0902 <strong>24\xB010'38" \u0932\u093E\u0939\u093F\u0930\u0940</strong>), \u091C\u093F\u0938\u0938\u0947 \u0917\u094D\u0930\u0939\u094B\u0902 \u0915\u0940 \u0938\u091F\u0940\u0915 \u0916\u0917\u094B\u0932\u0940\u092F \u0938\u094D\u0925\u093F\u0924\u093F \u092A\u094D\u0930\u093E\u092A\u094D\u0924 \u0939\u094B\u0924\u0940 \u0939\u0948\u0964` : `Vedic Astrology uses the <strong>Sidereal Zodiac</strong>, fixed to physical stellar constellations. Earth's axis precesses ~1\xB0 every 72 years (~25,772-year Great Year). The difference between Tropical 0\xB0 Aries and Sidereal 0\xB0 Aries is the <em>Ayanamsha</em> (currently <strong>24\xB010'38" Lahiri</strong>), ensuring planetary longitudes match exact astronomical positions.`;
    ...
    factCards[1].querySelector("p").innerHTML = isHi ? "360\xB0 \u0915\u0947 \u0938\u0902\u092A\u0942\u0930\u094D\u0923 \u0930\u093E\u0936\u093F \u091A\u0915\u094D\u0930 \u0915\u094B 27 \u091A\u0902\u0926\u094D\u0930 \u0928\u0915\u094D\u0937\u0924\u094D\u0930\u094B\u0902 \u092E\u0947\u0902 13\xB020' \u0915\u0947 \u092D\u093E\u0917\u094B\u0902 \u092E\u0947\u0902 \u0935\u093F\u092D\u093E\u091C\u093F\u0924 \u0915\u093F\u092F\u093E \u0917\u092F\u093E \u0939\u0948\u0964 \u091C\u0928\u094D\u092E \u0915\u0947 \u0938\u092E\u092F \u091A\u0902\u0926\u094D\u0930\u092E\u093E \u091C\u093F\u0938 \u0928\u0915\u094D\u0937\u0924\u094D\u0930 \u092E\u0947\u0902 \u0938\u094D\u0925\u093F\u0924 \u0939\u094B\u0924\u093E \u0939\u0948, \u0935\u0939 \u0906\u092A\u0915\u093E <em>\u091C\u0928\u094D\u092E \u0928\u0915\u094D\u0937\u0924\u094D\u0930</em> \u0915\u0939\u0932\u093E\u0924\u093E \u0939\u0948 \u0914\u0930 \u0935\u0939\u0940 \u0906\u092A\u0915\u0947 120-\u0935\u0930\u094D\u0937\u0940\u092F <strong>\u0935\u093F\u0902\u0936\u094B\u0924\u094D\u0924\u0930\u0940 \u0926\u0936\u093E \u091A\u0915\u094D\u0930</strong> \u0915\u093E \u0906\u0930\u0902\u092D \u092C\u093F\u0902\u0926\u0941 \u0924\u092F \u0915\u0930\u0924\u093E \u0939\u0948\u0964" : "The 360\xB0 zodiac is divided into 27 lunar mansions (<strong>Nakshatras</strong>) of 13\xB020' each. The Moon visits one Nakshatra daily. Your birth Moon's Nakshatra degree sets your <em>Janma Nakshatra</em> and unlocks your 120-year <strong>Vimshottari Dasha</strong> sequence \u2014 charting major timing windows of life.";
    ...
    factCards[2].querySelector("p").innerHTML = isHi ? "\u091C\u0928\u094D\u092E \u0915\u0941\u0902\u0921\u0932\u0940 (D1) \u091C\u0940\u0935\u0928 \u0915\u0947 \u092D\u094C\u0924\u093F\u0915 \u092A\u094D\u0930\u0915\u091F\u0940\u0915\u0930\u0923 \u0915\u094B \u0926\u0930\u094D\u0936\u093E\u0924\u0940 \u0939\u0948, \u091C\u092C\u0915\u093F \u092A\u094D\u0930\u0924\u094D\u092F\u0947\u0915 \u0930\u093E\u0936\u093F \u0915\u094B 9 \u0938\u0942\u0915\u094D\u0937\u094D\u092E \u092D\u093E\u0917\u094B\u0902 (3\xB020') \u092E\u0947\u0902 \u092C\u093E\u0902\u091F\u0915\u0930 <strong>\u0928\u0935\u093E\u0902\u0936 (D9)</strong> \u0915\u0941\u0902\u0921\u0932\u0940 \u092C\u0928\u0924\u0940 \u0939\u0948\u0964 \u092F\u0939 \u0917\u094D\u0930\u0939\u094B\u0902 \u0915\u0947 \u0938\u0942\u0915\u094D\u0937\u094D\u092E \u092C\u0932 (<em>\u0935\u0930\u094D\u0917\u094B\u0924\u094D\u0924\u092E</em>), \u0935\u0948\u0935\u093E\u0939\u093F\u0915 \u0938\u0941\u0916 \u0914\u0930 30 \u0935\u0930\u094D\u0937 \u0915\u0940 \u0906\u092F\u0941 \u0915\u0947 \u092C\u093E\u0926 \u092A\u0930\u093F\u092A\u0915\u094D\u0935 \u0939\u094B\u0928\u0947 \u0935\u093E\u0932\u0940 \u0906\u0924\u094D\u092E-\u0915\u094D\u0937\u092E\u0924\u093E \u0915\u094B \u0926\u0930\u094D\u0936\u093E\u0924\u0940 \u0939\u0948\u0964" : "While the birth chart (D1) represents physical manifestation, each sign is subdivided into 9 harmonic arcs of 3\xB020' to form the <strong>Navamsha (D9)</strong> chart. It reveals planetary micro-dignity (<em>Vargottama</em>), marital harmony, and inner spiritual potential maturing after age 30.";
    ...
    factCards[3].querySelector("p").innerHTML = isHi ? "12 \u092D\u093E\u0935 \u091C\u0940\u0935\u0928 \u0915\u0947 \u0938\u092D\u0940 \u0915\u094D\u0937\u0947\u0924\u094D\u0930\u094B\u0902 (\u0932\u0917\u094D\u0928 \u0938\u0947 10\u0935\u0947\u0902 \u0915\u0930\u094D\u092E \u092D\u093E\u0935 \u0924\u0915) \u0915\u094B \u0938\u092E\u0947\u091F\u0947 \u0939\u0941\u090F \u0939\u0948\u0902\u0964 <strong>\u0915\u0947\u0902\u0926\u094D\u0930 \u092D\u093E\u0935</strong> (1, 4, 7, 10) \u091C\u0940\u0935\u0928 \u0915\u0947 \u0938\u0941\u0926\u0943\u0922\u093C \u0906\u0927\u093E\u0930 \u0938\u094D\u0924\u0902\u092D \u0939\u0948\u0902, \u091C\u092C\u0915\u093F <strong>\u0924\u094D\u0930\u093F\u0915\u094B\u0923 \u092D\u093E\u0935</strong> (1, 5, 9) \u0932\u0915\u094D\u0937\u094D\u092E\u0940 \u0915\u0940 \u0915\u0943\u092A\u093E, <em>\u092A\u0942\u0930\u094D\u0935 \u092A\u0941\u0923\u094D\u092F</em> \u0914\u0930 \u091C\u094D\u091E\u093E\u0928 \u0935 \u0935\u093F\u0935\u0947\u0915 \u0915\u093E \u092A\u094D\u0930\u0924\u0940\u0915 \u0939\u0948\u0902\u0964" : "The 12 houses span life from vitality (1st / Lagna) to career (10th). The <strong>Kendra houses</strong> (1, 4, 7, 10) form the structural pillars of destiny, while the <strong>Trikona houses</strong> (1, 5, 9) signify Lakshmi's divine grace, past-life merit (<em>Purva Punya</em>), and wisdom.";
    ...
    factCards[4].querySelector("p").innerHTML = isHi ? "\u091C\u092C \u092A\u0943\u0925\u094D\u0935\u0940 \u0915\u093F\u0938\u0940 \u092C\u093E\u0939\u0930\u0940 \u0917\u094D\u0930\u0939 \u0915\u094B \u092A\u093E\u0930 \u0915\u0930\u0924\u0940 \u0939\u0948, \u0924\u094B \u0935\u0939 \u0917\u094D\u0930\u0939 \u092A\u0940\u091B\u0947 \u0915\u0940 \u0913\u0930 \u091A\u0932\u0924\u093E \u092A\u094D\u0930\u0924\u0940\u0924 \u0939\u094B\u0924\u093E \u0939\u0948, \u091C\u093F\u0938\u0947 <strong>\u0935\u0915\u094D\u0930\u0940 (\u0935\u0915\u094D\u0930) \u0917\u0924\u093F</strong> \u0915\u0939\u0924\u0947 \u0939\u0948\u0902\u0964 \u092A\u093E\u0930\u093E\u0936\u0930\u0940 \u091C\u094D\u092F\u094B\u0924\u093F\u0937 \u092E\u0947\u0902 \u0935\u0915\u094D\u0930\u0940 \u0917\u094D\u0930\u0939 \u0905\u0924\u094D\u092F\u0927\u093F\u0915 <em>\u091A\u0947\u0937\u094D\u091F\u093E \u092C\u0932</em> (\u092A\u094D\u0930\u092F\u093E\u0938 \u092C\u0932) \u092A\u094D\u0930\u093E\u092A\u094D\u0924 \u0915\u0930\u0924\u0947 \u0939\u0948\u0902, \u091C\u094B \u0917\u0939\u0928 \u0915\u0930\u094D\u092E \u092A\u0941\u0928\u0930\u094D\u092E\u0942\u0932\u094D\u092F\u093E\u0902\u0915\u0928 \u0914\u0930 \u0935\u093F\u0936\u093F\u0937\u094D\u091F \u092A\u094D\u0930\u0924\u093F\u092D\u093E \u0915\u093E \u0938\u0902\u0915\u0947\u0924 \u0926\u0947\u0924\u0947 \u0939\u0948\u0902\u0964" : "When Earth overtakes an outer planet, the planet appears to move backward in <strong>Retrograde (Vakra)</strong> motion. In Parashari astrology, retrograde planets gain high <em>Cheshta Bala</em> (effort strength), indicating deep karmic re-evaluation, intensity, and unconventional talent.";
    ...
    factCards[5].querySelector("p").innerHTML = isHi ? "\u0935\u0948\u0926\u093F\u0915 \u0938\u0902\u092C\u0902\u0927 \u092E\u093F\u0932\u093E\u0928 \u091C\u0928\u094D\u092E \u091A\u0902\u0926\u094D\u0930\u092E\u093E \u0915\u0947 \u0906\u0927\u093E\u0930 \u092A\u0930 \u090A\u0930\u094D\u091C\u093E\u0935\u093E\u0928 \u0938\u093E\u092E\u0902\u091C\u0938\u094D\u092F \u0915\u0947 8 \u0906\u092F\u093E\u092E\u094B\u0902 \u0915\u093E \u092E\u0942\u0932\u094D\u092F\u093E\u0902\u0915\u0928 \u0915\u0930\u0924\u093E \u0939\u0948\u0964 \u092A\u094D\u0930\u092E\u0941\u0916 \u0918\u091F\u0915\u094B\u0902 \u092E\u0947\u0902 <strong>\u0917\u094D\u0930\u0939 \u092E\u0948\u0924\u094D\u0930\u0940</strong> (\u092E\u093E\u0928\u0938\u093F\u0915 \u092E\u093F\u0924\u094D\u0930\u0924\u093E - 5 \u0905\u0902\u0915), <strong>\u092F\u094B\u0928\u093F</strong> (\u0938\u094D\u0935\u092D\u093E\u0935 \u0905\u0928\u0941\u0915\u0942\u0932\u0924\u093E - 4 \u0905\u0902\u0915), <strong>\u092D\u0915\u0942\u091F</strong> (\u092D\u093E\u0935\u0928\u093E\u0924\u094D\u092E\u0915 \u0924\u0930\u0902\u0917 - 7 \u0905\u0902\u0915), \u0914\u0930 <strong>\u0928\u093E\u0921\u093C\u0940</strong> (\u0938\u094D\u0935\u093E\u0938\u094D\u0925\u094D\u092F \u0935 \u0906\u0928\u0941\u0935\u0902\u0936\u093F\u0915 \u0938\u0902\u0924\u0941\u0932\u0928 - 8 \u0905\u0902\u0915) \u0936\u093E\u092E\u093F\u0932 \u0939\u0948\u0902\u0964" : "Vedic relationship matching evaluates 8 dimensions of energetic harmony between lunar birth positions. Key metrics include <strong>Graha Maitri</strong> (mental friendship - 5 pts), <strong>Yoni</strong> (instinctive compatibility - 4 pts), <strong>Bhakoot</strong> (emotional wavelength - 7 pts), and <strong>Nadi</strong> (genetic &amp; physiological balance - 8 pts).";
  }
  const factsFooterQuote = document.querySelector(".facts-footer-quote p");
  if (factsFooterQuote) {
    factsFooterQuote.textContent = isHi ? "\u201C\u0938\u092E\u0938\u094D\u0924 \u092C\u094D\u0930\u0939\u094D\u092E\u093E\u0902\u0921 \u0914\u0930 \u0909\u0938\u0915\u0947 \u091A\u0915\u094D\u0930 \u0917\u094D\u0930\u0939\u094B\u0902 \u0915\u0947 \u0905\u0927\u0940\u0928 \u0939\u0948\u0902 \u2014 \u091C\u094D\u092F\u094B\u0924\u093F\u0937 \u0935\u093F\u0926\u094D\u092F\u093E \u0907\u0928 \u091A\u0915\u094D\u0930\u094B\u0902 \u0915\u094B \u0938\u091A\u0947\u0924 \u090F\u0935\u0902 \u0938\u093E\u0930\u094D\u0925\u0915 \u091C\u0940\u0935\u0928 \u091C\u0940\u0928\u0947 \u0915\u093E \u0926\u0930\u094D\u092A\u0923 \u092E\u093E\u0928\u0924\u0940 \u0939\u0948\u0964\u201D" : "\u201CThe universe and its rhythms move under celestial influence \u2014 astrology observes these patterns as a mirror for conscious living.\u201D";
  }
  const howToReadHeadB = document.querySelector(".how-to-read-guide b");
  if (howToReadHeadB)
    howToReadHeadB.textContent = isHi ? "\u0905\u092A\u0928\u0940 \u0915\u0941\u0902\u0921\u0932\u0940 \u0915\u0948\u0938\u0947 \u0938\u092E\u091D\u0947\u0902 \xB7 4 \u0938\u0930\u0932 \u091A\u0930\u0923" : "HOW TO READ YOUR KUNDLI \xB7 4 EASY STEPS";
  const howToReadHeadP = document.querySelector(".how-to-read-guide p");
  if (howToReadHeadP)
    howToReadHeadP.textContent = isHi ? "\u092D\u093E\u0935, \u0917\u094D\u0930\u0939 \u0914\u0930 \u0926\u0936\u093E \u091A\u0915\u094D\u0930\u094B\u0902 \u0915\u094B \u0938\u092E\u091D\u0928\u0947 \u0915\u093E \u0938\u0930\u0932 \u092A\u094D\u0930\u093E\u0930\u0902\u092D\u093F\u0915 \u092E\u093E\u0930\u094D\u0917\u0926\u0930\u094D\u0936\u0915\u0964" : "A simple beginner's guide to navigating your houses, planets and dasha cycles.";
  ...
  if (reportHead)
    reportHead.textContent = isHi ? "\u0906\u092A\u0915\u0940 \u091C\u0928\u094D\u092E \u0915\u0941\u0902\u0921\u0932\u0940 \u0935 \u092B\u0932\u0915\u0925\u0928" : "Your reading";
  const progressHead = document.getElementById("progressHeading");
  if (progressHead)
    progressHead.textContent = isHi ? "\u0915\u0941\u0902\u0921\u0932\u0940 \u0917\u0923\u0928\u093E \u091C\u093E\u0930\u0940 \u0939\u0948\u2026" : "Casting the chart";
  const chartVisualH3 = document.querySelector("#chartVisualCard h3");
  if (chartVisualH3)
    chartVisualH3.textContent = isHi ? "\u0938\u091A\u093F\u0924\u094D\u0930 \u0935\u0948\u0926\u093F\u0915 \u091C\u0928\u094D\u092E \u0915\u0941\u0902\u0921\u0932\u0940" : "Visual Kundli Chart";
  ...
  if (chartCaption) {
    chartCaption.textContent = isHi ? "\u0909\u092A\u0930\u094B\u0915\u094D\u0924 \u0915\u0941\u0902\u0921\u0932\u0940 \u0907\u0938 \u0905\u0927\u094D\u092F\u092F\u0928 \u092E\u0947\u0902 \u092A\u094D\u0930\u092F\u0941\u0915\u094D\u0924 \u092A\u094D\u0930\u0924\u094D\u092F\u0915\u094D\u0937 \u0917\u094D\u0930\u0939\u0940\u092F \u0917\u0923\u0928\u093E \u0938\u0947 \u0928\u093F\u0930\u094D\u092E\u093F\u0924 \u0915\u0940 \u0917\u0908 \u0939\u0948\u0964 \u092D\u093E\u0935 \u092B\u0932\u0915\u0925\u0928 \u0915\u093E \u0935\u093F\u0935\u0930\u0923 \u0928\u0940\u091A\u0947 \u0932\u093F\u0916\u093F\u0924 \u0916\u0902\u0921\u094B\u0902 \u092E\u0947\u0902 \u0926\u093F\u092F\u093E \u0917\u092F\u093E \u0939\u0948\u0964 \u0917\u094D\u0930\u0939 \u0938\u094D\u0925\u093F\u0924\u093F\u092F\u093E\u0902 \u0932\u093E\u0939\u093F\u0930\u0940 \u0928\u093F\u0930\u092F\u0923 \u0916\u0917\u094B\u0932 \u0917\u0923\u0928\u093E \u092A\u0930 \u0906\u0927\u093E\u0930\u093F\u0924 \u0939\u0948\u0902\u0964" : "The chart above is generated directly from the structured planetary placements used in this reading. Interpretive meaning is explained in the written sections below. Planetary positions displayed here come from the connected Lahiri sidereal ephemeris calculation; the AI layer is not allowed to invent or alter degrees.";
  }
  const repGlossaryH3 = document.querySelector("#glossaryCard h3");
  if (repGlossaryH3)
    repGlossaryH3.textContent = isHi ? "\u092A\u093E\u0930\u093F\u092D\u093E\u0937\u093F\u0915 \u091C\u094D\u092F\u094B\u0924\u093F\u0937 \u0936\u092C\u094D\u0926\u093E\u0935\u0932\u0940" : "Jyotish Glossary";
  const repGlossaryGrid = document.querySelector("#glossaryCard .glossary-grid");
  if (repGlossaryGrid) {
    repGlossaryGrid.innerHTML = isHi ? `<span><b>\u0932\u0917\u094D\u0928</b><small>\u091C\u0928\u094D\u092E \u0915\u0947 \u0938\u092E\u092F \u092A\u0942\u0930\u094D\u0935 \u0915\u094D\u0937\u093F\u0924\u093F\u091C \u092A\u0930 \u0909\u0926\u093F\u0924 \u0930\u093E\u0936\u093F \u090F\u0935\u0902 \u092D\u093E\u0935\u094B\u0902 \u0915\u093E \u0906\u0930\u0902\u092D \u092C\u093F\u0902\u0926\u0941\u0964</small></span>
         ...
         <span><b>\u0926\u0936\u093E</b><small>\u0918\u091F\u0928\u093E\u0913\u0902 \u0915\u0947 \u0938\u091F\u0940\u0915 \u0915\u093E\u0932 \u0928\u093F\u0930\u094D\u0927\u093E\u0930\u0923 \u0939\u0947\u0924\u0941 \u0917\u094D\u0930\u0939\u0940\u092F \u0915\u093E\u0932\u0916\u0902\u0921\u0964</small></span>` : `<span><b>Lagna</b><small>Ascendant and starting point of the houses.</small></span>
  ...
  if (gateKicker)
    gateKicker.textContent = isHi ? "\u0917\u0939\u0928 \u0915\u0941\u0902\u0921\u0932\u0940 \u092A\u094D\u0930\u091C\u094D\u091E\u093E" : "Deeper Chart Intelligence";
  const gateH3 = document.querySelector(".premium-gate h3");
  if (gateH3)
    gateH3.textContent = isHi ? "\u0938\u0902\u092A\u0942\u0930\u094D\u0923 \u091C\u0928\u094D\u092E \u0915\u0941\u0902\u0921\u0932\u0940 \u0905\u0927\u094D\u092F\u092F\u0928 \u0905\u0928\u0932\u0949\u0915 \u0915\u0930\u0947\u0902" : "Unlock the Complete Reading";
  const gateP = document.querySelector(".premium-gate p:not(.premium-note)");
  if (gateP) {
    gateP.textContent = isHi ? "\u0935\u093F\u0938\u094D\u0924\u0943\u0924 \u0930\u093E\u091C\u092F\u094B\u0917, \u0938\u0902\u092A\u0942\u0930\u094D\u0923 120-\u0935\u0930\u094D\u0937\u0940\u092F \u0935\u093F\u0902\u0936\u094B\u0924\u094D\u0924\u0930\u0940 \u0926\u0936\u093E \u0905\u0928\u0941\u0915\u094D\u0930\u092E, \u091C\u0940\u0935\u0928-\u091A\u0930\u0923 \u0935\u093F\u0936\u094D\u0932\u0947\u0937\u0923, \u0938\u0942\u0915\u094D\u0937\u094D\u092E \u0915\u093E\u0932-\u0928\u093F\u0930\u094D\u0927\u093E\u0930\u0923 \u0914\u0930 \u0938\u0902\u092A\u0942\u0930\u094D\u0923 \u092A\u094D\u0930\u0936\u094D\u0928-\u0909\u0924\u094D\u0924\u0930 \u0905\u0928\u0941\u092D\u0935 \u092A\u094D\u0930\u093E\u092A\u094D\u0924 \u0915\u0930\u0947\u0902\u0964" : "Unlock the extended interpretation including detailed Yogas, full Vimshottari Dasha sequencing, life-phase analysis, advanced timing, deeper relationship patterns and the complete Ask the Chart experience.";
  }
  const gatePoints = document.querySelector(".premium-gate .gate-points");
  if (gatePoints) {
    gatePoints.innerHTML = isHi ? "<span>\u0935\u093F\u0938\u094D\u0924\u0943\u0924 \u0930\u093E\u091C\u092F\u094B\u0917</span><span>\u092E\u0939\u093E\u0926\u0936\u093E \u0935 \u0905\u0902\u0924\u0930\u094D\u0926\u0936\u093E</span><span>\u091C\u0940\u0935\u0928-\u091A\u0930\u0923 \u092B\u0932</span><span>\u0938\u091F\u0940\u0915 \u0915\u093E\u0932-\u0928\u093F\u0930\u094D\u0927\u093E\u0930\u0923</span><span>\u0905\u0938\u0940\u092E\u093F\u0924 \u092A\u094D\u0930\u0936\u094D\u0928-\u0909\u0924\u094D\u0924\u0930</span>" : "<span>Detailed Yogas</span><span>Mahadasha &amp; Antardasha</span><span>Life-phase interpretation</span><span>Advanced timing</span><span>Full Ask the Chart</span>";
  }
  const premUnlockBtn = document.getElementById("premiumUnlockBtn");
  if (premUnlockBtn)
    premUnlockBtn.textContent = isHi ? "\u092A\u094D\u0930\u0940\u092E\u093F\u092F\u092E \u0905\u0927\u094D\u092F\u092F\u0928 \u0905\u0928\u0932\u0949\u0915 \u0915\u0930\u0947\u0902 \xB7 \u20B959" : "Unlock Premium Reading";
  const chatHead = document.getElementById("chatHeading");
  if (chatHead)
    chatHead.textContent = isHi ? "\u0905\u092A\u0928\u0940 \u0915\u0941\u0902\u0921\u0932\u0940 \u0938\u0947 \u091C\u0941\u0921\u093C\u0947 \u092A\u094D\u0930\u0936\u094D\u0928 \u092A\u0942\u091B\u0947\u0902" : "Ask about this reading";
  const chatStatusSpan = document.querySelector(".chat-status > span:first-child");
  if (chatStatusSpan) {
    chatStatusSpan.textContent = isHi ? "\u0915\u0941\u0902\u0921\u0932\u0940, \u0926\u0936\u093E, \u092F\u094B\u0917, \u0938\u0902\u092C\u0902\u0927, \u0915\u0930\u093F\u092F\u0930 \u092F\u093E \u091C\u0940\u0935\u0928 \u091A\u0930\u0923\u094B\u0902 \u0915\u0947 \u092C\u093E\u0930\u0947 \u092E\u0947\u0902 \u0935\u093F\u0936\u093F\u0937\u094D\u091F \u092A\u094D\u0930\u0936\u094D\u0928 \u092A\u0942\u091B\u0947\u0902\u0964" : "Ask specific questions about the chart, Dasha, Yogas, relationships, career, or life phases. Each question costs \u20B929.";
  }
  const chatInput = document.getElementById("chatInput");
  if (chatInput)
    chatInput.placeholder = isHi ? "\u0905\u092A\u0928\u0940 \u0915\u0941\u0902\u0921\u0932\u0940 \u0938\u0947 \u0938\u0902\u092C\u0902\u0927\u093F\u0924 \u092A\u094D\u0930\u0936\u094D\u0928 \u092F\u0939\u093E\u0901 \u0932\u093F\u0916\u0947\u0902\u2026" : "Type a question about your chart\u2026";
  const chatSendBtn = document.getElementById("chatSend");
  if (chatSendBtn)
    chatSendBtn.textContent = isHi ? "\u092A\u0942\u091B\u0947\u0902" : "Ask";
  const chatHint = document.getElementById("chatHint");
  if (chatHint) {
    chatHint.textContent = isHi ? "\u0915\u0941\u0902\u0921\u0932\u0940 \u0915\u0940 \u0917\u0923\u0928\u093E \u0939\u094B \u0930\u0939\u0940 \u0939\u0948 \u2014 \u092A\u0939\u0932\u093E \u0916\u0902\u0921 \u092A\u094D\u0930\u0915\u091F \u0939\u094B\u0924\u0947 \u0939\u0940 \u0906\u092A \u0905\u092A\u0928\u0947 \u092A\u094D\u0930\u0936\u094D\u0928 \u092A\u0942\u091B \u0938\u0915\u0924\u0947 \u0939\u0948\u0902\u0964" : "The chart is being cast \u2014 you can start asking questions the moment the first section appears above.";
  }
  const endReadingBtn = document.getElementById("endReadingBtn");
  if (endReadingBtn)
    endReadingBtn.textContent = isHi ? "\u0928\u092F\u093E \u0905\u0927\u094D\u092F\u092F\u0928 \u092A\u094D\u0930\u093E\u0930\u0902\u092D \u0915\u0930\u0947\u0902" : "End reading";
  const mysticBtn = document.getElementById("mysticSoundBtn");
  if (mysticBtn) {
    const isPlaying = window.OmChantEngine && window.OmChantEngine.isPlaying;
  ...
  if (feedbackBtnFixed)
    feedbackBtnFixed.textContent = isHi ? "\u2726 \u0938\u0941\u091D\u093E\u0935 \u0935 \u0938\u0902\u092A\u0930\u094D\u0915" : "\u2726 Feedback";
  const accessBtn = document.getElementById("accessBtn");
  if (accessBtn)
    accessBtn.innerHTML = isHi ? "<span>\u2301</span> \u0917\u094B\u092A\u0928\u0940\u092F \u0932\u0949\u0917\u093F\u0928" : "<span>\u2301</span> Private access";
  if (typeof renderPlacementTable === "function")
    renderPlacementTable();
  if (typeof renderInterpretationTable === "function")
    renderInterpretationTable();
  if (typeof renderDashaTimeline === "function")
    renderDashaTimeline();
  if (typeof renderClassicalModules === "function")
    renderClassicalModules();
  if (typeof buildAtAGlance === "function")
    buildAtAGlance();
  if (typeof renderDailyRashifal === "function")
    renderDailyRashifal(currentPanchangDate);
  if (typeof window.applyPricingToUI === "function")
    window.applyPricingToUI(window.SERVER_CONFIG);
};
...
window.updateEphemerisTimer = function() {
  ...
  if (agoEl) {
    const diffSec = Math.max(0, Math.floor((Date.now() - window.lastEphemerisFetchTimestamp) / 1e3));
    if (diffSec < 5) {
...
setInterval(window.updateEphemerisTimer, 1e3);
const originalLoadCurrentSky = window.loadCurrentSky;
window.loadCurrentSky = function() {
    ...
    displayCard.innerHTML = `
      <div style="background:rgba(0,0,0,0.35);border:1px solid rgba(216,160,76,0.3);border-radius:10px;padding:14px 18px;margin-top:12px;animation:fadeIn 0.25s ease;">
        <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;margin-bottom:8px;">
          <b style="font-family:'Cinzel',serif;font-size:14px;color:${item.color};">\u263D Moon in ${item.sign} \xB7 ${item.house}</b>
...
window.printReportDoc = function() {
  ...
  if (covName)
    covName.textContent = name || "Native";
  const covGender = document.getElementById("covGender");
  if (covGender)
    covGender.textContent = gender || "Unspecified";
  const covDob = document.getElementById("covDob");
  if (covDob)
    covDob.textContent = dob || "\u2014";
  const covTob = document.getElementById("covTob");
  if (covTob)
    covTob.textContent = tob || "\u2014";
  const covPob = document.getElementById("covPob");
  if (covPob)
    covPob.textContent = pob || "\u2014";
  const covCoords = document.getElementById("covCoords");
  if (covCoords) {
    covCoords.textContent = lat && lon ? `${Number(lat).toFixed(4)}\xB0 N, ${Number(lon).toFixed(4)}\xB0 E \xB7 Lahiri Sidereal (Chitrapaksha)` : "Lahiri Sidereal (Chitrapaksha)";
  }
  ...
  if (window.lastVerifiedChart && window.lastVerifiedChart.normalized) {
    ...
    if (covLagna)
      covLagna.textContent = `${norm.ascendant.rashiName} (${norm.ascendant.degreesFormatted}) \xB7 ${norm.ascendant.nakshatra} Pada ${norm.ascendant.pada}`;
    if (covMoon) {
      const mo = norm.planets.find((p) => p.key === "Mo" || p.key === "Moon");
      if (mo)
        covMoon.textContent = `${mo.rashiName} (${mo.degreesFormatted}) \xB7 ${mo.nakshatra} Pada ${mo.pada}`;
    }
    ...
    if (covLagna)
      covLagna.textContent = s.Ascendant || "\u2014";
    if (covMoon)
      covMoon.textContent = s.Moon || "\u2014";
  }
