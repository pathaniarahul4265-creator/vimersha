...
async function aiCall({model, systemText, userText, maxTokens, sessionToken, vipToken}){
  // Enable validation by un-commenting later if desired, right now we just allow pass-through.
...
async function aiStreamCall(req, res, {model, systemText, userText, maxTokens, sessionToken, vipToken}){
  const isValid = await validatePremiumSession(sessionToken, vipToken);
  if (!isValid) {
    const e = new Error('Valid Premium Session or VIP Code required');
    e.status = 403;
  ...
  throw new Error('AI stream request could not be completed after rotating key pool.');
}
