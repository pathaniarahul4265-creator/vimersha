const fs = require('fs');
let code = fs.readFileSync('public/vedic-engine.js', 'utf8');
...
// Replace rules in code (matching from const RULES = `...`;)
code = code.replace(/const RULES = `[\s\S]*?`;/, newRules);
code = code.replace(/const KUNDLI_RULES = `[\s\S]*?`;/, newKundli);

fs.writeFileSync('public/vedic-engine.js', code);
console.log("public/vedic-engine.js updated.");
